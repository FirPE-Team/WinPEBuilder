...
if "x%findcmd%"=="xfindstr" (
    findstr /i /c:"%~1" "%APP_TMP_PATH%\AddDrivers_INF.txt" >nul
) else (
    find /i "%~1" "%APP_TMP_PATH%\AddDrivers_INF.txt" >nul
)
if not "%errorlevel%"=="0" (
...
\Windows\System32\DriverStore\%APP_PE_LANG%\%_AddDriver_INF%_loc
:end_files
set _AddDriver_INF=
set _AddDriver_Name=
